...
PID_FILE="$RUNTIME_DIR/hermes-switchui.pid"
LOG_FILE="$RUNTIME_DIR/hermes-switchui.log"
BUILD_LOG_FILE="$RUNTIME_DIR/hermes-switchui.build.log"
mkdir -p "$RUNTIME_DIR"
...
echo "[stable] building Hermes Switch UI..."
rm -rf dist
pnpm build >"$BUILD_LOG_FILE" 2>&1

echo "[stable] starting Hermes Switch UI on port $PORT..."
