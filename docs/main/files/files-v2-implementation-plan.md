...
- **Phase 0 — tokens & icons.** Add `--accent` (default green) + kind-color map to `matrix-files.css`. Port `IcoV2` SVG icon set into a small `files-icons.tsx`; swap emoji glyph buttons for it. Kind-colored tree/pill tint. _Check: page renders unchanged in structure, new icons + colors visible._
- **Phase 1 — search upgrade.** Add `fuzzy()` + `<Highlight>`; replace substring filter. Wire ↑/↓/Enter/Esc row keyboard nav. _Check: fuzzy match + highlight, keyboard selects rows._
- **Phase 2 — sort + type filter.** Sort menu (name/modified/type) over existing tree; type filter from client kind derivation. _Check: each sort/filter reorders correctly._
- **Phase 3 — command palette.** Cmd/Ctrl+K overlay, fuzzy over flat `FILE_INDEX`-equivalent (flatten our tree), Enter selects. _Check: palette opens, jumps to file._
- **Phase 4 — Quick Access.** Pin toggle + Recents as collapsible sidebar groups (`.qa`); persist to localStorage per workspace. _Check: pin survives reload, recents populate on open._
- **Phase 5 — outline scrollspy** for markdown preview (active heading on scroll).
- **Phase 6 — Tweaks panel.** Accent picker (4 accents → `--accent`), row density (`--rowpad`), label toggle, show/hide Quick Access. Files-local, persisted to localStorage. _Check: each control mutates the page live and survives reload._
...
- **Pre-release gate — DONE (2026-07-24).** Browser smoke at 1440×900 loaded `/files` with the Files v2 shell visible, Quick jump, type-filter group, and folder listing present; no browser console errors or uncaught page errors. The empty-workspace and file-load timeout states also rendered without layout breakage.
- **Production build — PASS (2026-07-24).** `pnpm --dir website build` and `HERMES_SKIP_GATEWAY_BOOT=1 pnpm exec vite build` both completed. The first website attempt hit a stale generated Astro manifest; clearing only `website/dist` resolved it, with no source change required.
...
- Fresh 2026-07-24 rerun: `pnpm vitest run src/screens/files/files-screen.test.tsx src/screens/chat/components/sidebar/v2/sidebar-shell-v2.test.tsx src/components/file-explorer/file-explorer-sidebar.test.tsx` → 6 pass / 0 fail.
- **Pre-release status: COMPLETE.** Visual/browser QA and production builds pass; no Files v2 source changes were needed in the final gate.
