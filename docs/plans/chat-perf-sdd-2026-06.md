...
- Phase 7 (C1) — GREEN (static) — commit `5c1a8cb9`. CONTAINMENT approach (user chose B over full store-slice A). Root cause: live streaming text (~25fps post-P5) threaded as prop through ChatMessageList broke its memo every frame → whole list subtree reconciled per frame (heavy pipeline itself already stable from P1-3/#212). Fix: new `StreamingTextContext` + `StreamingMessageItem` leaf (`streaming-text-context.tsx`); chat-screen provides live text via context; only the single live-bubble leaf consumes it + re-renders per frame, feeding MessageItem's existing reveal machinery unchanged. ChatMessageList now takes stable `hasStreamingText` boolean (flips ~once/stream) for presence checks (typing indicator/grace/bottom-pin/empty placeholder) + dropped `streamingText` from memo comparator → no longer per-frame re-render. Store untouched. Two streaming render paths converted (renderMessage site + inline-map site). `MessageItemProps` exported. tsc chat files clean, repo baseline 32 unchanged; 46 chat tests pass. Mechanical multi-site edits delegated to sonnet executor (no stash incident; tree was pre-committed). Live-confirm PENDING: smooth reveal + typing indicator + no dup/flash on finalize + profiler no per-frame parent re-render.

  - Phase 7 LIVE-CONFIRM (Canary sonnet 2026-06-18): bubble NOT broken — thinking indicator appears, exactly ONE assistant message (no dup), no flash/flicker/jump on finalize, final text complete + correct, composer responsive (46ms) while streaming. CAVEAT: Canary's seconds-coarse polling saw text appear all-at-once (model thought ~20s then emitted a block); sub-second smooth word reveal likely fell between polls. Refactor doesn't touch MessageItem reveal machinery or smooth-hook output, so token-by-token granularity should be unchanged — user to eyeball. Transient backend 503s + a brief session-not-found 404 during new-session creation (pre-existing, self-resolved), no JS console errors.
- Phase 8 (C2, optional) — DEFERRED. Phase 7 containment landed clean with no evidence it's insufficient (heavy pipeline already stable from P1-3/#212; chat-screen no longer forces ChatMessageList per frame). Revisit only if live profiling still shows lag under streaming.

## Status: 7 of 8 phases GREEN (Phase 8 deferred as designed). Branch perf/chat-sidebar-composer.
