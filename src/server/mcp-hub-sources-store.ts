...
function readUserSources(): Promise<{ sources: Array<HubSourceEntry>; error?: string; validationErrors?: Array<ValidationIssue> }> {
  ...
  const stat = statKey(path)
  if (!stat.ok) {
    if (stat.missing) return Promise.resolve({ sources: [] })
    return Promise.resolve({ sources: [], error: `cannot read hub-sources file: ${stat.code}` })
  }

  let text: string | null = null
  try { text = readFileSync(path, 'utf8') } catch { /* ignore */ }
  if (!text) return Promise.resolve({ sources: [] })

  let parsed: unknown
  try {
    parsed = JSON.parse(text)
  } catch (err) {
    return Promise.resolve({ sources: [], error: `not valid JSON: ${(err as Error).message}` })
  }

  const validation = validatePayload(parsed)
  const hardErrors = validation.errors.filter((e) => !e.message.includes('unknown top-level field'))
  if (hardErrors.length > 0) {
    return Promise.resolve({ sources: validation.sources, error: 'validation errors', validationErrors: hardErrors })
  }
  return Promise.resolve({ sources: validation.sources })
}
