...
import { Route as ProfileRoute } from './profiles.$profile'
...
describe('GET /api/self-improve/profiles/$profile', () => {
  it('returns readable paused state', async () => {
    mockIsAuthenticated.mockReturnValue(true)
    mockGetProfileStatus.mockResolvedValue({ profile: 'default', paused: true })
    const req = makeRequest(
      'GET',
      'http://localhost/api/self-improve/profiles/default',
    )
    const res: Response = await profileHandlers.GET({
      request: req,
      params: { profile: 'default' },
    })
    expect(await res.json()).toEqual({ profile: 'default', paused: true })
  })
})
...
describe('GET /api/self-improve/scenarios', () => {
  it('returns 401 when not authenticated', async () => {
    ...
    const req = makeRequest(
      'GET',
      'http://localhost/api/self-improve/scenarios?profile=default',
    )
    const res: Response = await scenariosHandlers.GET({
      request: req,
      params: {},
    })
  ...
  it('passes include_holdout param through to client', async () => {
    ...
    const req = makeRequest(
      'GET',
      'http://localhost/api/self-improve/scenarios?profile=default&include_holdout=1',
    )
...
describe('DELETE /api/self-improve/scenarios/$id', () => {
  ...
  it('returns 400 for invalid id', async () => {
    ...
    const req = makeRequest(
      'DELETE',
      'http://localhost/api/self-improve/scenarios/abc',
    )
    const res: Response = await scenarioIdHandlers.DELETE({
      request: req,
      params: { id: 'abc' },
  ...
  it('deletes and returns ok', async () => {
    ...
    const req = makeRequest(
      'DELETE',
      'http://localhost/api/self-improve/scenarios/5',
    )
    const res: Response = await scenarioIdHandlers.DELETE({
      request: req,
      params: { id: '5' },
  ...
  it('returns 404 when scenario not found', async () => {
    ...
    mockDeleteScenario.mockRejectedValue(
      new Error('Self-Improve API error 404: not found'),
    )
    const req = makeRequest(
      'DELETE',
      'http://localhost/api/self-improve/scenarios/99',
    )
    const res: Response = await scenarioIdHandlers.DELETE({
      request: req,
      params: { id: '99' },
...
describe('POST /api/self-improve/profiles/$profile/pause', () => {
  it('returns 401 when not authenticated', async () => {
    ...
    const req = makeRequest(
      'POST',
      'http://localhost/api/self-improve/profiles/default/pause',
      {},
    )
    const res: Response = await pauseHandlers.POST({
      request: req,
      params: { profile: 'default' },
    })
    expect(res.status).toBe(401)
  ...
  it('pauses profile and returns ok + paused=true', async () => {
    mockIsAuthenticated.mockReturnValue(true)
    mockPauseProfile.mockResolvedValue({
      ok: true,
      profile: 'default',
      paused: true,
    })
...
describe('POST /api/self-improve/profiles/$profile/resume', () => {
  it('returns 401 when not authenticated', async () => {
    ...
    const req = makeRequest(
      'POST',
      'http://localhost/api/self-improve/profiles/default/resume',
      {},
    )
    const res: Response = await resumeHandlers.POST({
      request: req,
      params: { profile: 'default' },
    })
    expect(res.status).toBe(401)
  ...
  it('resumes profile and returns ok + paused=false', async () => {
    mockIsAuthenticated.mockReturnValue(true)
    mockResumeProfile.mockResolvedValue({
      ok: true,
      profile: 'default',
      paused: false,
    })
