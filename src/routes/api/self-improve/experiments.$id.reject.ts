...
import { rejectExperiment } from '../../../server/self-improve-client'

export const Route = createFileRoute('/api/self-improve/experiments/$id/reject')({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        try {
          body = (await request.json()) as { actor?: string; reason?: string }
        } catch {
        ...
        try {
          const result = await rejectExperiment(id, actor, body.reason)
