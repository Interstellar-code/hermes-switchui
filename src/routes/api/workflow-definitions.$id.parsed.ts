...
function nodeType(
  node: DagNode
): 'prompt' | 'bash' | 'command' | 'approval' | 'loop' | 'router' | 'cancel' | 'script' {
    ...
    handlers: {
      GET: async ({ request, params }) => {
        ...
        for (const node of nodes) {
          const deps = (node as { depends_on?: string[] }).depends_on ?? [];
        ...
        const projectedNodes = nodes.map(node => {
          ...
          return {
            ...
            type: nodeType(node),
            depends_on: (node as { depends_on?: string[] }).depends_on ?? [],
            skills: (raw['skills'] as string[] | undefined) ?? [],
            model_hint: (raw['model'] as string | undefined) ?? null,
