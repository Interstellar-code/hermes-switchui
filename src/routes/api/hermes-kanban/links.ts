    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (typeof body.parent_id !== 'string' || typeof body.child_id !== 'string') {
          return Response.json({ error: 'parent_id and child_id are required' }, { status: 400 })
        }
        try {
      ...
      DELETE: async ({ request }) => {
        ...
        if (!parentId || !childId) {
          return Response.json({ error: 'parent_id and child_id query params are required' }, { status: 400 })
        }
        try {
