...
// ── fs mock ──────────────────────────────────────────────────────────────────
const { existsSync, renameSync } = vi.hoisted(() => ({
  existsSync: vi.fn().mockReturnValue(false),
  renameSync: vi.fn().mockImplementation(() => {}),
}))

vi.mock('node:fs', () => ({
  default: { existsSync, renameSync },
  existsSync,
  renameSync,
}))
...
function makeRequest(body: unknown): Request {
  return new Request('http://localhost/api/profiles/trash-restore', {
    method: 'POST',
...
async function getHandler() {
  vi.resetModules()
  const mod = await import('../trash-restore')
  return (
    mod as unknown as {
      Route: {
        server: {
...
describe('POST /api/profiles/trash-restore', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('restores and returns the original name', async () => {
    ...
    expect(body).toEqual({ ok: true, name: 'jarvis' })
    expect(renameSync).toHaveBeenCalled()
  })

  it('returns 409 when the target profile already exists', async () => {
    const id = `jarvis-${Date.now()}`
    existsSync.mockReturnValue(true) // both trashed source AND target "exist"
    const handler = await getHandler()
    const res = await handler({ request: makeRequest({ id }) })
