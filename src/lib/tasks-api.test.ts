...
describe('tasks-api (Agent Kanban backend)', () => {
  it('fetchTasks calls /api/hermes-kanban/board and flattens columns', async () => {
    mockFetch.mockResolvedValueOnce(ok({
      board: {
        columns: {
          triage: [{ id: 't1', title: 'T1', status: 'triage', priority: 0, spawn_failures: 0 }],
          todo: [],
          ready: [],
          running: [],
          blocked: [],
  ...
  it('createTask posts to /api/hermes-kanban/tasks with Agent fields', async () => {
    mockFetch.mockResolvedValueOnce(ok({ task: { id: 'new', title: 'New', status: 'triage', priority: 0, spawn_failures: 0 } }, 201))
  ...
  it('moveTask sends PATCH with status to /api/hermes-kanban/tasks/:id', async () => {
    mockFetch.mockResolvedValueOnce(ok({ task: { id: 't2', status: 'running', priority: 0, spawn_failures: 0 } }))
  ...
  it('deleteTask sends PATCH status:archived (archive not hard-delete)', async () => {
    mockFetch.mockResolvedValueOnce(ok({ task: { id: 't3', status: 'archived', priority: 0, spawn_failures: 0 } }))
