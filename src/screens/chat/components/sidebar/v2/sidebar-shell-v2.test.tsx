...
vi.mock('@/screens/chat/apply-filters-and-decorate', () => ({ applyFiltersAndDecorate }))
vi.mock('@/stores/sessions-filter-store', () => ({
  useSessionsFilterStore: (selector: (state: Record<string, unknown>) => unknown) =>
    selector({
      collapsed: false, setCollapsed: vi.fn(), leftPanel: 'sessions', setLeftPanel: vi.fn(),
      sources: ['chat'], query: 'needle', dateRange: { from: null, to: null }, sort: 'recent',
    }),
}))
vi.mock('@/stores/sessions-local-store', () => ({
  useSessionsLocalStore: (selector: (state: Record<string, unknown>) => unknown) =>
    selector({ pinned: [], starred: [], archived: [] }),
}))
...
vi.mock('./sidebar-source-chips-v2', () => ({ SidebarSourceChipsV2: () => null }))

beforeEach(() => {
  useSessionsFeed.mockReturnValue({ items: [{ id: 'chat:a' }, { id: 'task:b' }], sources: [] })
  applyFiltersAndDecorate.mockReturnValue({ groups: [], totalCount: 0, sourceCounts: {} })
})
...
it('passes the raw merged feed to the filtering owner', () => {
  ...
  expect(applyFiltersAndDecorate).toHaveBeenCalledWith(
    [{ id: 'chat:a' }, { id: 'task:b' }],
    expect.objectContaining({ sources: ['chat'], query: 'needle' }),
